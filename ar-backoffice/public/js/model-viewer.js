...
AFRAME.registerComponent('model-viewer', {
  ...
  init: function () {
    ...
    var gui = this.gui = new dat.GUI({ autoPlace: true });
    var params = this.params ={
      scale: 1.0
    };
    
    var scaling = this.scaling = false;
    var scaleController = this.scaleController = gui.add(this.params,"scale", 0,1).step(0.0001).onChange(function(newValue) {
      scaling = true;
      console.log("Value changed to:  ", newValue);
      modelEl.object3D.scale.set(newValue, newValue, newValue);
    });

    this.scaleController.listen();

    if(this.data.guiCont){
      var customContainer = document.getElementById(this.data.guiCont);
      console.log("pinche ui", this, this.el);
      //
  ...
  centerAndScaleModel: function () {
    ...
    console.log("PArams", this.params, scale);
    this.params.scale = scale;
    this.scaleController.max(scale * 4);
    this.scaleController.step(scale * 0.1);
